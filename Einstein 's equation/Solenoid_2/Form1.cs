    ...
    public partial class Form1 : Form
    {
        ...
        private void solve_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (textBox_work.Text.All<char>(Char.IsDigit) == false)
                {
                    textBox_work.Clear();
                    throw new Exception("Указан невозможная работа выхода фотоэлектронов!");
                }
                if (double.Parse(textBox_work.Text) < 0)
                {
                    textBox_work.Clear();
                    throw new Exception("Указан неверная работа выхода фотоэлектронов!");
                }
                work = double.Parse(textBox_work.Text);//* 10^(-19)
                double rate = c / length;//* 10^17
                if (energy(rate, work) < 0)
                {
                    label_final.Text = "0";
                }
                else
                {
                    label_final.Text = energy(rate, work).ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private double energy(double rate, double work)
        {
            return (rate * h - work / 100) * 100;// (10^(-17) - 10^(-19) / 100) * 100
        }
        ...
        private void graphics_button_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (textBox_param1_gr.Text.All<char>(Char.IsDigit) == false)
                {
                    ...
                    throw new Exception("Указан невозможная работа выхода!");
                }
                if (double.Parse(textBox_param1_gr.Text) <= 0)
                {
                    textBox_param1_gr.Clear();
                    throw new Exception("Указан неверная работа выхода!");
                }
                work = double.Parse(textBox_param1_gr.Text);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message + "\nПроверьте введёные данные!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            Series mySeriesOfPoint = new Series("Кинетическая энергия фотоэлектронов");
            mySeriesOfPoint.ChartType = SeriesChartType.Line;
            mySeriesOfPoint.ChartArea = chart.ChartAreas[0].Name;
            Axis ax = new Axis();
            ax.Title = "Работа выхода (Дж 10^(-19))";
            chart.ChartAreas[0].AxisX = ax;
            Axis ay = new Axis();
            ay.Title = "Кинетическая энергия (Дж 10^(-19))";
